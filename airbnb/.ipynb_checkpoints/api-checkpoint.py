...
class Api(object):
    ...
    @randomizable
    def get_calendar(self, listing_id, starting_month=datetime.datetime.now().month, starting_year=datetime.datetime.now().year, calendar_months=12,timeout=1):
        """
        ...
        r = self._session.get(API_URL + "/calendar_months", params=params,timeout=timeout)
        r.raise_for_status()

        return r.json()

    @randomizable
    def get_reviews(self, listing_id, offset=0, limit=20,timeout=1):
        """
    ...
    @randomizable
    def get_homes(self, query=None, gps_lat=None, gps_lng=None, checkin=None, checkout=None, offset=0, items_per_grid=8,timeout=1):
        """
        ...
        r = self._session.get(API_URL + '/explore_tabs', params=params,timeout=timeout)
        r.raise_for_status()

        return r.json()

    @randomizable
    def get_listing_details(self, listing_id,timeout=1):
        params = {
            'adults': '0',
            '_format': 'for_native',
            'infants': '0',
        ...
        r = self._session.get(API_URL + '/pdp_listing_details/' + str(listing_id), params=params,timeout=timeout)
        r.raise_for_status()
